    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="text-center">
                <?php
        try {
            // Requête pour sélectionner toutes les tâches de la base de données
            $sql = "SELECT * FROM Task
                    JOIN User ON Task.id_user = User.id
                    JOIN State ON Task.id_state = State.id
                    JOIN Priority ON Task.id_priority = Priority.id WHERE id = Task.id";
                    ...
                    if ($stmt->rowCount() > 0) {
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            echo '<p class="card-text">Priorité: ' . $row['priority'] . '</p>';
                            echo '<p class="card-text">Date de création: ' . $row['creation_date'] . '</p>';
                            echo '<p class="card-text">Échéance: ' . $row['end'] . '</p>';
                            echo '<a href="more.php?id=' . $row['id'] . '" class="btn btn-info"><i class="fas fa-arrow-left"></i>  retour</a>';
